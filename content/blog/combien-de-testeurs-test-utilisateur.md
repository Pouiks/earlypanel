...
### « Quelle version est la meilleure ? » : une comparaison

Vous hésitez entre deux parcours, deux formulations, deux structures de page. Ici, chaque participant ne voit qu'une version, ou voit les deux dans un ordre alterné. Il faut donc au minimum **doubler le panel**, soit une vingtaine de participants pour une comparaison à deux branches, et davantage si les différences attendues sont subtiles.

Avant de lancer ce type de test, posez-vous une question honnête : la différence entre les deux versions est-elle assez grande pour qu'un test qualitatif la détecte ? Si vous comparez deux nuances d'un même bouton, la réponse est non, et un test A/B en production sur du trafic réel fera mieux, moins cher.
...
## Les questions à se poser avant de fixer le chiffre

- **Combien de segments distincts dans ma cible ?** Comptez trois à quatre personnes par segment que vous voulez vraiment observer.
- **Combien de scénarios ?** Au-delà de quatre ou cinq scénarios, la fatigue dégrade les réponses de fin de test. Mieux vaut deux vagues plus courtes qu'une longue.
- **Est-ce une première version ou un produit mature ?** Sur une première version, les problèmes sont gros et visibles : huit personnes les trouveront. Sur un produit déjà poli, les frictions restantes sont plus fines et plus dispersées : allez vers douze.
- **Quelle est ma capacité de relecture ?** Chaque testeur supplémentaire, c'est autant de réponses à lire sérieusement. Un panel qu'on n'a pas le temps de relire ne vaut rien.

## Ce qui compte plus que le nombre

Le nombre de participants est la variable la plus visible et la moins déterminante. Trois choses pèsent davantage :

1. **La qualité du recrutement.** Dix personnes qui ressemblent à vos clients valent mieux que trente volontaires tirés d'un panel généraliste. Le métier, la situation et l'équipement réel font la différence, pas l'effectif.
