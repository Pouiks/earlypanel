...
- **Filtrer les participants** avec les critères disponibles, et accepter que certains ne ressemblent pas vraiment à votre cible.
- **Lire chaque réponse ou regarder chaque vidéo**, en écarter les inutilisables, comparer, synthétiser.
- **Rédiger et présenter** des conclusions que l'équipe acceptera.

Pour une équipe qui a un UX researcher et teste chaque mois, cette organisation est logique : l'outil amplifie une compétence interne. Pour une équipe produit sans researcher, qui teste deux ou trois fois par an, c'est là que les tests se perdent : le projet est lancé, les réponses arrivent, personne n'a le temps de les lire sérieusement, et le rapport n'est jamais écrit.

### Ce qu'un service clé en main prend en charge
...
- **Une restitution en visio** pour arbitrer avec l'équipe.

De votre côté : deux réunions et un accès au produit. Le reste est fait.
...
C'est la différence la moins visible et la plus importante. Dans un test à distance, une partie des réponses est inutilisable : trop courtes, hors sujet, expédiées. Si elles sont comptées, les chiffres du test ne veulent rien dire. Un service dont la relecture fait partie du forfait a intérêt à être exigeant ; un outil qui vous laisse relire vous laisse aussi la tentation de ne pas le faire.

### Le livrable

Des données brutes, des vidéos et des tableaux demandent encore plusieurs jours de travail avant de devenir une décision. Un rapport rédigé se lit en vingt minutes et se présente en comité tel quel. Selon que vous avez ou non quelqu'un pour faire cette synthèse, l'écart de prix entre les deux modèles se justifie ou non.

### Le modèle de prix

Une plateforme se paie à l'usage ou à l'abonnement, indépendamment de ce que vous en tirez. Un service se paie au forfait par mission, résultat compris. Nous détaillons ce qui fait varier ces prix dans [combien coûte un test utilisateur](/blog/prix-test-utilisateur).

## Comment choisir

**Choisissez une plateforme si** vous avez une personne dont c'est le métier, un rythme de tests régulier, une cible large, et le temps de relire vous-même. L'outil vous rendra plus productif.

**Choisissez un service clé en main si** vous n'avez pas de UX researcher, si votre cible est précise ou professionnelle, si vous testez ponctuellement autour d'une décision importante (lancer un développement, ouvrir un produit, corriger un funnel), et si ce dont vous avez besoin est un rapport, pas un outil.

Les deux peuvent coexister : une plateforme pour les vérifications rapides du quotidien, un service pour les tests qui engagent une décision.
