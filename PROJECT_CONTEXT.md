...
- Depouillement (section `review`, 2026-09-09) : une question a la fois, navigateur de questions par scenario, tri (non lus, plus courtes, ordre de soumission), reponses repliees une fois lues, note du testeur posee sur place (meme PATCH `/answers`). « Lu » persiste dans `project_tester_answers.reviewed_at/reviewed_by` (migration 042, `PATCH /api/staff/projects/[id]/answers/reviewed`), sans effet metier. Compteur de caracteres = chiffre brut, pas de scoring automatique (decision fondateur).
...
| **Refus de mission notifié** (`src/lib/mission-refusal-email.ts`, route `answers` PATCH) | Quand le staff coche « travail bâclé » à la première notation, le testeur reçoit un email : mission non payée, note staff transmise comme motif (l'UI de notation le signale, placeholder « Motif du refus »), baisse de score, contestation en répondant à l'email ou à contact@. Échec d'envoi tracé dans l'audit (`refusal_email: sent/failed/skipped`), jamais bloquant. CGU signées passées en v1.2-2026-09 avec l'article 4 « Validation et refus » (resignature demandée à la prochaine modification d'IBAN). FAQ /testeurs et /cgu alignées. Tests : `tests/unit/mission-refusal-email.test.ts`. | ✅ Implémenté |
| **Infobulles glossaire** (`src/lib/glossary-inline.ts`, `src/components/ui/glossify.tsx`, `GlossaryTerm.tsx`) | Chaque terme de `src/data/glossaire.ts` qui déclare des `aliases` est reconnu dans les textes (paragraphes et listes uniquement, jamais titres / boutons / nav) et devient un lien `/glossaire#slug` + infobulle au survol, focus ou premier appui tactile. Une seule annotation par terme et par page : Set mémoïsé par requête (`React.cache`) côté serveur, Set local + prop `skip` dans `ArticleBody`. Les mots-clés des pages piliers (« test utilisateur », « testeur rémunéré ») n'ont pas d'alias et ne sont jamais annotés. La définition n'est pas dans le HTML servi (rendue à l'ouverture). Tests : `tests/unit/glossary-inline.test.ts`. | ✅ Implémenté |
